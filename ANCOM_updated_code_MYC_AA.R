...
ancom.W.myc = function(otu_data,var_data,
  ...
  for ( i in otu_ids) {
    aveRelAbund[i,] <- aggregate(otu_data_relAbund[,i], by=list(data_comp_sorted[[main.var]]), FUN=mean)[,"x"] # Alex change: used mean instead of median relative abundance per main.var factor
